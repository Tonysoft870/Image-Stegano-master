...
public class CannyEdgeDetector {
   ...
   private final static float GAUSSIAN_CUT_OFF = 0.005f;
	...
	private float[] yGradient;
        ...
        public String resultWithPadding ="";
	...
	private void thresholdEdges() {
		for (int i = 0; i < picsize; i++) {
			data[i] = data[i] > 0 ? -1 : 0xff000000;
                       //  System.out.println("Edge color : " + data[i]);
		}
                 int bin= data[picsize-1];
                 String bn1 = Integer.toString(bin);
            BigInteger bigInt = new BigInteger(bn1);      
            byte b1[]; 
    ...
    //                                  + "0x%02X", 
    //                              b1[i]); 
                 resultWithPadding = String.format(" "+ "0x%02X",   b1[i]);
